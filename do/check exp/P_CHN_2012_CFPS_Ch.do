...
Preparation file for China, 2012
	...
	Instrument: p70-74, P153-158 in "CFPS 2012 questionnaire.pdf"
	
	hexp sepc: avaliable in health and expense module. In health module: For ind age >=16, total IP, OOP-IP, total OP, total IPexp on medical treatment, total IPexp on non-medical (food, nursary care, lodging) recall in 12 month. 
...
use "${SOURCE}/WB MICRO DATA/`refid'/Data/ecfps2012adult_201906.dta", clear				
	...
	foreach var of varlist qp506* qp508* qp507* qp501followuptotal qp509a qp510 qp511 qp512{
	...
	* IP cost on food, nursary care, lodging
	...
	* Total IP cost, not limited to medical treatment, food, nursary care, and lodging
	gen hexp_IP = qp501followuptotal 
	* out-of-pocket total IP expense 
	gen hexp_OOPIP = qp509a	
	
	* total hexp exclude IP 
	gen hexp_OP = qp510		
	* total hexp for ind pay no IP visit
	gen hexp_total = qp511  
	* Not data, out-of-pocket total hexp for ind pay no IP visit
	gen hexp_OOPOP = qp512 	
	
	* hh_hexp 
	gen hexp_adult = hexp_OOPIP + hexp_OP 								// out-of-pocket total IP expense + OP hexp
	sum hexp_adult if   qp501 !=1 // 0 
	replace hexp_adult = hexp_total 		if   qp501 !=1				// total hexp (for hh pay no IP visit)
	* hh_hexp_alt 
	gen hexp_adult_alt = hexp_IPmed + hexp_OP      						// total IP expense on medical treatment  + tptal OP hexp
	sum hexp_adult_alt if qp501 !=1 // 0 
	
	replace hexp_adult_alt = hexp_total  	if   qp501 !=1				// total hexp (from subquestion for hh pay no IP visit)
...
*************
	* Total health expenditure aggregates
	gen hh_hexp = hexp				// for ind aged>=16, out-of-pocket IP expense + OP hexp, for ind aged<=15, out-of-pocket hexp 
	* Hexp Alt 1
	gen hh_hexp_alt = hexp_alt     // for ind aged>=16, total IPexp on medical treatment + OP hexp, for ind aged<=15, out-of-pocket hexp 
...
*spec
	...
	gen byte IP_itemN = 25														// number of items
...
gen healthshare_alt = hh_hexp_alt/hh_exp
gen foodshare = hh_fexp/hh_exp
sum hh_hexp* hh_exp	*share healthshare_alt
xx
*/						
